...
# Low-ionization lines used to pin the "good" alpha_ox range at solar Z.
# At fixed solar metals, the alpha_ox values where the model curves match
# the VdB observed bands for these lines define a narrow vertical strip;
# the plot overlays that strip in every panel as the "consistent SED" zone.
LOW_ION_KEYS = ['MgII', 'CIII', 'CII']
LOW_ION_LOG_Z_REF = 0.0  # solar metallicity slice for the constraint
...
# Constantin & Shields composite emission-line measurements (Table 3,
# relative flux = 100*F/F(Lyalpha)). CII] 2326 is not in the published
# table.
CSH = {
    'NV':   (13.0,  1.5),
    'NIV':  (0.7,   0.3),
...
def observed_ratio(num_key, denom_key='CIV', source=VDB):
    """Return (ratio, err) for source[num]/source[denom] with quadrature
    error propagation. Returns (None, None) if either line is missing.
    """
    if num_key not in source or denom_key not in source:
        return None, None
    n, ne = source[num_key]
    d, de = source[denom_key]
    r = n / d
...
def low_ion_aox_range(records, log_z_ref=LOW_ION_LOG_Z_REF,
                      keys=LOW_ION_KEYS, n_fine=2001):
    """At the slice closest to log_z_ref (solar by default), find the alpha_ox
    range where each low-ionization line/CIV ratio matches the VdB observed
    band. Return (lo, hi) = the union envelope of the per-line crossings.

    The VdB error bars are very small, so the per-line crossings are narrow
    points around the curve crossings. Taking the union (min lo, max hi)
    gives a slightly wider band that all three lines fall inside.
    """
    unique_logZ = np.unique([r['logZ'] for r in records])
    isol = int(np.argmin(np.abs(unique_logZ - log_z_ref)))
    logZ_sol = unique_logZ[isol]

    sel = sorted([r for r in records if np.isclose(r['logZ'], logZ_sol)],
                 key=lambda r: r['aox'])
    aox = np.array([r['aox'] for r in sel])
    aox_fine = np.linspace(aox.min(), aox.max(), n_fine)

    los, his = [], []
    for key in keys:
        y = np.array([r['lines'][key] / r['lines']['CIV'] for r in sel])
        obs, err = observed_ratio(key, 'CIV')
        yfine = np.interp(aox_fine, aox, y)
        in_band = (yfine >= obs - err) & (yfine <= obs + err)
        if not in_band.any():
            continue
        los.append(aox_fine[in_band].min())
        his.append(aox_fine[in_band].max())

    if not los:
        return None
    return (min(los), max(his)), logZ_sol
...
def make_figure(records):
    ...
    low_ion_result = low_ion_aox_range(records)
    if low_ion_result is None:
        aox_lo = aox_hi = None
        logZ_pin = LOW_ION_LOG_Z_REF
        print('No alpha_ox range matches any low-ion line at solar Z.')
    else:
        (aox_lo, aox_hi), logZ_pin = low_ion_result
        print(f'Low-ion-consistent alpha_ox at log Z = {logZ_pin:.2f}: '
              f'[{aox_lo:.3f}, {aox_hi:.3f}]')
    ...
    for ax, (key, ylabel) in zip(axes.flat, panels):
        ...
        # Observed BLR bands: VdB01 (red) and Constantin & Shields (blue)
        r_obs, r_err = observed_ratio(key, 'CIV', source=VDB)
        ...
        r_csh, e_csh = observed_ratio(key, 'CIV', source=CSH)
        if r_csh is not None:
            ax.axhspan(r_csh - e_csh, r_csh + e_csh, color='royalblue',
                       alpha=0.20, zorder=0)
            ax.axhline(r_csh, color='royalblue', ls='--', lw=2.0, zorder=2,
                       label=r'C\&S obs.')

        # Vertical band: alpha_ox range pinned by low-ion lines at solar Z
        if aox_lo is not None:
            ax.axvspan(aox_lo, aox_hi, color='black', alpha=0.15, zorder=0,
                       label=(r'low-ion-consistent $\alpha_{\rm ox}$ '
                              r'($Z_\odot$)'))

        # Put the y-axis identifier inside the panel as a top text box, so
        # adjacent panels' tick labels do not overlap with neighbouring
        # ylabels.
        ax.text(0.97, 0.97, ylabel, transform=ax.transAxes,
                ha='right', va='top', fontsize=15,
                bbox=dict(boxstyle='round,pad=0.25', facecolor='white',
                          edgecolor='lightgrey', alpha=0.85))
        ax.set_yscale('log')
        # Per-panel ymax override to leave room for the in-panel label.
        ymax_override = {'NIV': 0.1}
        if key in ymax_override:
            ymin, _ = ax.get_ylim()
            ax.set_ylim(ymin, ymax_override[key])
        ax.minorticks_on()
